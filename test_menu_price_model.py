...
class TestMenuPriceOptimizer(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.test_data['current_price'] = np.maximum(
            self.test_data['current_price'],
            self.test_data['cogs'] * 1.2
        )
    ...
    def test_calculate_portfolio_metrics(self):
        ...
        required_keys = ['mean_return', 'volatility', 'sharpe_ratio', 'recommendations', 'num_items']
...
class TestModelPerformance(unittest.TestCase):
    ...
    def test_large_dataset_performance(self):
        ...
        large_data = pd.DataFrame({
            ...
            'cogs': np.random.uniform(5, 25, n_samples),
            'quantity_sold': np.random.randint(10, 200, n_samples),
            'category': np.random.choice(['Appetizer', 'Main', 'Dessert'], n_samples),
        })
        
        large_data['current_price'] = np.maximum(
            large_data['current_price'],
            large_data['cogs'] * 1.2
        )
