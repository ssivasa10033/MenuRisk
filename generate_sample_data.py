...
if __name__ == "__main__":
    ...
    for category in sorted(df["category"].unique()):
        ...
        print(f"  {category:12s}: ${price_min:.2f} - ${price_max:.2f}")

    total_revenue = (df["current_price"] * df["quantity_sold"]).sum()
    total_cogs = (df["cogs"] * df["quantity_sold"]).sum()
    gross_profit = ((df["current_price"] - df["cogs"]) * df["quantity_sold"]).sum()
    avg_margin = ((df["current_price"] - df["cogs"]) / df["current_price"]).mean()

    print(f"\nTotal revenue: ${total_revenue:,.2f}")
    print(f"Total COGS: ${total_cogs:,.2f}")
    print(f"Gross profit: ${gross_profit:,.2f}")
    print(f"Avg margin: {avg_margin * 100:.1f}%")
