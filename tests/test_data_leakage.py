...
class TestTimeSeriesFeatureEngineer:
    ...
    def test_no_future_information_in_features(self, sample_timeseries_data):
        ...
        test_mean_feature = test_df[test_df["item_name"] == "Item_A"][
            "item_mean_train"
        ].iloc[0]
    ...
    def test_rolling_features_use_past_only(self, sample_timeseries_data):
        ...
        test_row = features[
            (features["date"] == test_date) & (features["item_name"] == item)
        ].iloc[0]
    ...
    def test_lag_features_use_past_only(self, sample_timeseries_data):
        ...
        expected_lag = df[(df["date"] == lag_date) & (df["item_name"] == item)][
            "quantity_sold"
        ].iloc[0]
    ...
    def test_item_statistics_from_training_only(self, sample_timeseries_data):
        ...
        expected_mean = train_df[train_df["item_name"] == "Item_A"][
            "quantity_sold"
        ].mean()
        ...
        assert (
            abs(stored_mean - full_mean) > abs(stored_mean - expected_mean)
            or abs(stored_mean - expected_mean) < 1e-6
    ...
    def test_fit_then_transform_maintains_separation(self, sample_timeseries_data):
        ...
        test_item_mean = transformed_test[transformed_test["item_name"] == "Item_B"][
            "item_mean_train"
        ].iloc[0]

        expected_train_mean = train_df[train_df["item_name"] == "Item_B"][
            "quantity_sold"
        ].mean()

        assert (
            abs(test_item_mean - expected_train_mean) < 1e-6
    ...
    def test_new_items_handled_gracefully(self, sample_timeseries_data):
        ...
        assert (
            not transformed["item_mean_train"].isna().all()
...
class TestTrainTestSplit:
    ...
    def test_split_by_date_not_random(self, timeseries_data):
        ...
        X_train, X_test, y_train, y_test, engineer = create_train_test_split(
            timeseries_data, test_size_days=30
        )

        # Get original dates
        train_dates = timeseries_data[timeseries_data["date"] <= engineer.cutoff_date][
            "date"
        ]
        test_dates = timeseries_data[timeseries_data["date"] > engineer.cutoff_date][
            "date"
        ]
    ...
    def test_target_not_in_features(self, timeseries_data):
        ...
        X_train, X_test, y_train, y_test, _ = create_train_test_split(
            timeseries_data, test_size_days=30, target_col="quantity_sold"
        )

        assert (
            "quantity_sold" not in X_train.columns
        ), "Target variable is in training features!"
        assert (
            "quantity_sold" not in X_test.columns
...
class TestFeatureValidity:
    ...
    def test_rolling_average_correctness(self, deterministic_data):
        ...
        assert (
            abs(actual_avg - expected_avg) < 0.5
    ...
    def test_profit_margin_calculation(self, deterministic_data):
        ...
        assert all(
            abs(m - expected_margin) < 1e-6 for m in actual_margins
