...
def generate_synthetic_menu_data(
    ...
    category_traits = {
        'Appetizer': {'cogs_range': (3, 8), 'base_demand': (15, 40), 'markup': (1.8, 2.5)},
        'Main': {'cogs_range': (8, 18), 'base_demand': (25, 60), 'markup': (2.0, 3.5)},
        'Dessert': {'cogs_range': (2, 6), 'base_demand': (10, 35), 'markup': (2.5, 4.0)},
        'Beverage': {'cogs_range': (1, 4), 'base_demand': (20, 50), 'markup': (3.0, 5.0)}
    }
    ...
    for date in dates:
        ...
        for item in items:
            ...
            if include_price_changes and np.random.random() < 0.05:  # 5% chance of price change
    ...
    logger.info(f"Avg daily sales per item: {df.groupby('date')['quantity_sold'].sum().mean():.1f}")
...
def save_sample_data(output_path: str = 'data/sample_menu_data_timeseries.csv'):
    ...
    print(df.groupby('item_name').agg({
