...
class Settings(BaseSettings):
    ...
    risk_free_rate: float = Field(
        default=0.0225,
        description="Bank of Canada overnight rate (Oct 2025)"
    )
    sharpe_keep_threshold: float = Field(
        default=1.5,
        description="Sharpe ratio threshold for KEEP recommendation"
    )
    sharpe_monitor_threshold: float = Field(
        default=0.8,
        description="Sharpe ratio threshold for MONITOR recommendation"
    )
    ...
    min_observations: int = Field(
        default=30,
        description="Minimum observations per item"
    )
    test_size_days: int = Field(
        default=30,
        description="Number of days for test set"
    )
    cv_splits: int = Field(
        default=5,
    ...
    charts_dir: str = Field(
        ...
        description="Directory for generated charts"
    )
    ...
    mlflow_experiment_name: str = Field(
        default='menurisk-demand-forecasting',
    ...
    # Canadian-specific
    default_province: str = Field(
        default='ON',
        description="Default province code"
    )
