...
def main():
    ...
    print(
        f"   Avg daily sales: {df.groupby('date')['quantity_sold'].sum().mean():.1f} units"
    )

    # Save sample for inspection
    os.makedirs("data", exist_ok=True)
    df.to_csv("data/workflow_sample_data.csv", index=False)
    print("   Saved to: data/workflow_sample_data.csv")
    ...
    print(
        f"Train period: {train_df['date'].min().date()} to {train_df['date'].max().date()}"
    )
    print(
    ...
    numeric_cols = (
        train_features[feature_cols].select_dtypes(include=[np.number]).columns.tolist()
    )
    ...
    for idx, row in top_5.iterrows():
        print(
            f"   {row['item_name']:25} | Sharpe: {row['sharpe_ratio']:6.2f} "
    ...
    optimizer = PriceOptimizer(
        demand_forecaster=forecaster, feature_engineer=feature_engineer
    )
    ...
    print(
        f"\n[WARNING] {len(significant_changes)} items need significant price changes (>5%):"
    )
    ...
    final_recommendations = optimized_menu.merge(
        risk_metrics_df[
            ["item_name", "sharpe_ratio", "recommendation", "volatility_daily"]
        ],
        on="item_name",
        how="left",
    )
    ...
    for category in ["KEEP", "MONITOR", "REMOVE"]:
        ...
        if len(items_in_category) > 0:
            for idx, item in items_in_category.head(3).iterrows():
                ...
                print(
                    f"    Sharpe: {item['sharpe_ratio']:5.2f} | "
    ...
    improvement_pct = (
        (profit_improvement / total_current_profit * 100)
