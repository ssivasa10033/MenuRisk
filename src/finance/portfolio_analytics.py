...
class PortfolioAnalyzer:
    ...
    def calculate_sharpe_ratio(self, returns: np.ndarray, freq: str = "daily") -> float:
        """
    ...
    def calculate_sortino_ratio(
        self, returns: np.ndarray, target_return: Optional[float] = None
    ) -> float:
        """
    ...
    def test_return_normality(self, returns: np.ndarray) -> Dict[str, Any]:
        ...
        logger.info(
            f"Normality test: Shapiro p={shapiro_p:.4f}, " f"is_normal={is_normal}"
        )

        return result

    def analyze_item(self, df: pd.DataFrame, item_name: str) -> Dict[str, Any]:
        """
        ...
        if len(item_df) < 14:
            logger.warning(
                f"{item_name} has <14 days of data, metrics may be unreliable"
            )
        ...
        if sharpe >= 1.5:  # type: ignore[operator]
            metrics["recommendation"] = "KEEP - Excellent risk-adjusted returns"
        elif sharpe >= 0.8:  # type: ignore[operator]
    ...
    def analyze_menu(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if "date" not in df.columns:
            raise ValueError(
                "DataFrame must have 'date' column for time-series analysis"
            )
    ...
    def calculate_portfolio_return_and_risk(
        self, df: pd.DataFrame, weights: Optional[Dict[str, float]] = None
    ) -> Dict[str, float]:
        """
        ...
        returns_pivot = df_with_returns.pivot_table(
            index="date", columns="item_name", values="daily_return"
        )
