...
class PortfolioAnalyzer:
    ...
    This class provides the old PortfolioAnalyzer interface while using
    the improved RiskMetrics implementation under the hood.

    DEPRECATED: Use RiskMetrics and PortfolioMetrics directly for new code.
    """
    ...
    def calculate_sharpe_ratio(
        ...
        """
        Calculate Sharpe ratio (non-annualized for backward compatibility).

        NOTE: This uses the old non-annualized formula for compatibility.
        For proper annualized Sharpe, use RiskMetrics.calculate_sharpe_ratio().
        """
        if len(returns) == 0:
    ...
    def calculate_portfolio_metrics(self, df: pd.DataFrame) -> Dict:
        """
        Calculate portfolio metrics using legacy interface.

        Args:
            df: DataFrame with current_price, cogs, quantity_sold columns

        Returns:
            Dict with mean_return, volatility, sharpe_ratio, recommendations, num_items
        """
        logger.debug("Calculating portfolio metrics (backward compatibility mode)")
        ...
        # Calculate profit margins using revenue-based definition
        ...
        df["profit_margin"] = np.where(
            df["revenue"] > 0, df["profit"] / df["revenue"], 0
        )
        ...
        # Generate per-item recommendations using CORRECT item volatility
        recommendations = {}
        for item_name in df["item_name"].unique():
            ...
            item_returns = item_returns[~np.isinf(item_returns)]
            ...
            if item_vol == 0:
                item_sharpe = item_mean - self.risk_free_rate
            else:
                item_sharpe = (item_mean - self.risk_free_rate) / item_vol
    ...
    def calculate_var(
        ...
        return self._risk_metrics.calculate_value_at_risk(
            returns, confidence=confidence_level
        )
    ...
    def get_efficient_frontier_points(
        ...
        """Generate efficient frontier points (simplified single-asset)."""
        mean_return = np.mean(returns)
